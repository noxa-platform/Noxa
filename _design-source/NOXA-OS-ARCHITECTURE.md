...
作成: 2026-05-30 / 更新: 2026-06-02（配信チャネル建付け確定）
ステータス: 構想 v2.1

---

## 0. 配信チャネルの建付け（2026-06-02 確定・最重要）

```
Web 版          = NOXA に完全統合（全機能 = 個人 + 店舗運営すべて）。ブランド NOXA
                  ↳ 実体は yorulog リポ（完成したアプリシェル (app)/ があるため）。リポは将来統一
よるログアプリ   = 売上管理専門（個人キャスト/ホスト向け、② 売上管理のみ）のネイティブアプリ
  ├ iOS        = 別リポ yorulog-ios（SwiftUI）。このワークスペースから触らない
  └ Android    = 現状 Capacitor WebView だが、Web が NOXA フルになると店舗機能も見えてしまう
                 → ネイティブ作り直しを検討中（保留）。当面は Web の NOXA 完全統合を優先
```

**決定の理由**: yorulog は元々「個人ユーザー（キャスト・ホスト）の売上管理」向け。そこに POS/勤怠/給与など
店舗運営機能を混ぜると役割が破綻する。そこで **配信チャネルで分離**：Web ブラウザ = NOXA OS フル、
ネイティブアプリ = 「よるログ」売上特化。1 コードベース（yorulog リポ = NOXA Web 本体）で、表示を出し分ける。

**実装上の含意**:
- 店舗運営モジュール（①③〜⑨）は yorulog リポの `(app)/` に置いてよい（Web では NOXA として出る）
- Firestore は既に `personal_*`（個人）/ `shop_shops`（店舗）/ `account_*`（共通）で分離済み。UI もこの境界に沿う
- 「よるログ」ブランド表記はネイティブアプリ専用。Web UI には出さない（NOXA 表記）
- リポ統一（noxa リポの LP/account を yorulog リポに統合 or 逆）は別フェーズの大工事。当面は並行
...
**NOXA は「夜の街の OS」**。
個人キャスト・ホストもオーナーも、夜職に必要な業務を **1 アカウント・1 画面** から動かせるプラットフォーム。POS から名刺発注まで、ナイトワークの店舗運営に必要な全機能を束ねた **Operating System** として位置付ける。**Web 版が NOXA OS の本体**（§0 参照）。
