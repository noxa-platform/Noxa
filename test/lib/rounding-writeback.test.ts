 ...
 *
 * ## 測ったうえで「足さない」と決めたこと（同じ調査をやり直さないために残す）
 * - **`STATUSES` を増やす向きの変種は測っていない。** ここが見るのは「未知の値を丸めていないか」で、
 *   union に値を足すのは**丸めの追加ではない**（型が全域を要求するので `Record` 側で落ちる）。
 *   ⇒ union に**未知を受ける穴**（`string` へ緩める等）を足したときは 1 通り足すこと。
 * - **`BusinessCardClient` / `InventoryClient` は「該当なし」をテストに残してある。**
 *   消すと次に見る人が同じ棚卸しをやり直す。⇒ 実装が変わったらここも直す前提の assert。
 * - **`raw-firestore-read.test.ts` の方には経路ごとの破壊測定を足していない。**
 *   あちらは**件数のラチェット**なので、1 箇所壊せば数が動いて必ず赤になる（形が違う）。
 */
