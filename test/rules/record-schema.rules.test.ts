...
// ⚠️ **それを守る「唯一の関門」も無い**（P154-PM4 で実測）。iOS は Firestore へ直接書き、
// Web の `validateXMap` には本番の呼び出し元が 0 件。**各書き手が自分の写しで守っている**。
// ⚠️ **写しが見ているのも*値*についてだけ**（P153-PM3）。差分の組み立て方——とくに「消す」という
