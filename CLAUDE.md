...
このリポは田口修平（個人事業主、屋号 egshugy）が運営する **夜職 DX プラットフォーム事業「Noxa」** の運用ナレッジを集約する。
...
```
田口修平（個人事業主、屋号 egshugy） ← 運営者
│
└── egshugy-lab （個人公式サイト・親サイト） ← 全プロダクトの入口
    │
    └── プロダクトライン
        ...
        │   └── nomishugy（バーポータル）
        │
        └── エグタイプ診断 （診断・ゲーム）   ← 別軸、ここでは扱わない
            └── kuzu-type / ramune-puzzle 他
```

- **Noxa 親リポ（このリポ）**: 夜職 DX 事業共通のブランド・ドキュメント
- **YoruLog / nomishugy**: 各プロダクトの実装は別リポ
- **egshugy-lab**: 個人公式サイト（親サイト、このリポ管理外）
- **エグタイプ診断**: 診断・ミニゲームライン（このリポ管理外）
...
- 法務情報（特商法）には「運営: 田口修平（個人事業主、屋号 egshugy）」と記載
...
### egshugy-lab
- 田口修平の個人公式サイト（親サイト・全プロダクト入口）
- プロフィール / プロダクト一覧 / ブログ / お問合せ
- 将来は `egshugy.com` を取得してメイン入口に
- 社内サーバ配信（Proxmox CT103 webserver = 192.168.0.77:3001）

### エグタイプ診断
- 診断・ミニゲーム集（kuzu-type、ramune-puzzle 他）
- Noxa とは別軸のコンテンツライン（同じ運営者）
...
- 親サイト: `egshugy.com`（個人公式 / プロダクト入口、取得予定）
- Noxa 事業: `noxa.com`（夜職 DX 事業サイト、取得予定）
- YoruLog: `yorulog.com`（CRM 製品サイト、取得予定）
- nomishugy: `nomishugy.com`（バーポータル、取得予定）
- 開発中の暫定: `yorulog.vercel.app`、`minami-bar-guide.firebaseapp.com`
...
3. egshugy-lab / エグタイプ診断 / Paperclip の作業 → 各リポで管理（このリポ外）
