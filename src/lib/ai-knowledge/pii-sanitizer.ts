...
// 電話番号（日本国内の一般的なフォーマット）。区切りは共通の PHONE_SEP_CLASS に集約
// （3ファイルで個別定義され区切り漏れが繰り返したため単一ソース化・Day51）。
const PHONE_RE = new RegExp(`(?:\\+?81${PHONE_SEP_CLASS}?|0)(?:\\d{1,4}${PHONE_SEP_CLASS}?){1,3}\\d{3,4}`, 'g');
...
// カタカナのグリフ範囲。ァ-ヶ(U+30A1-30F6) だけだと長音符「ー」(U+30FC)・中黒「・」を
// 含まないため、"ミラージュ" が "ミラー" で途切れる／"アリーさん" の氏名が伏字化されない等の
// 部分漏れが起きる。長音符・中黒はカタカナ店名/人名で多用されるので明示的に含める。
const KATA = 'ァ-ヶー・ヽヾ';

// 地名っぽいワード（都道府県・政令市＋主要駅名）は伏字化対象が広すぎるため、
// 代わりに「〇〇駅」「〇〇区」「〇〇市」パターンを検出
const LOCATION_RE = new RegExp(`[一-龠ぁ-ん${KATA}]{1,6}(駅|市|区|町|村|丁目|番地)`, 'g');

// 日本人氏名候補（2〜4文字の漢字 or カタカナ）: 誤検出多めなので接尾辞とのペアで限定
const NAME_RE = new RegExp(`([一-龠]{2,4}|[${KATA}]{2,6})(さん|くん|ちゃん|様|君|氏|先生|社長|部長|店長)`, 'g');

// 店舗名候補（「〇〇店」「〇〇クラブ」「〇〇ラウンジ」等）
// ※ 接頭辞が「お」「この」「その」等の一般的な代名詞・冠詞は除外して過剰マッチを防ぐ
const STORE_NAME_RE = new RegExp(`(?<![お御そこあど])[一-龠${KATA}A-Za-z0-9]{2,10}(店|クラブ|ラウンジ|スナック)`, 'g');
