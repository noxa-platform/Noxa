...
/**
 * 投稿系コンポーネント: スレッド作成 / 返信 / NG ワード警告ダイアログ。
 * 投稿前に findNgWords でチェックし、ヒット時は警告ダイアログで本人確認を取る。
 */

import { useState } from 'react';
import { FONT, WINE, WINE_INK } from '@/lib/community/constants';
import { findNgWords } from '@/lib/community/ng-words';
...
export function ThreadComposer({
  ...
  const trySubmit = () => {
    if (!canSubmit) return;
    const hits = findNgWords(`${title} ${body}`);
    if (hits.length > 0) { setWarn(hits); return; }
    submit();
  };
  ...
  return (
    <div style={cardBox()}>
      ...
      {warn && <NgWarning words={warn} onCancel={() => setWarn(null)} onProceed={() => { setWarn(null); submit(); }} />}
    </div>
  );
}
...
export function NgWarning({ words, onCancel, onProceed }: { words: string[]; onCancel: () => void; onProceed: () => void }) {
  return (
    <div role="alertdialog" aria-label="規約違反の可能性" style={{ position: 'fixed', inset: 0, background: 'rgba(7,5,13,0.7)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 60, padding: 20 }} onClick={onCancel}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: 'var(--noxa-surface-card)', border: `1px solid ${WINE}66`, borderRadius: 14, padding: 20, maxWidth: 380, width: '100%' }}>
        <div style={{ fontFamily: fontJp, fontSize: 15, fontWeight: 700, color: 'var(--noxa-text-primary)', marginBottom: 10 }}>この投稿は規約違反の可能性があります</div>
        <p style={{ fontFamily: fontJp, fontSize: 13, lineHeight: 1.7, color: 'var(--noxa-text-muted)', margin: '0 0 8px' }}>
          以下の語が含まれています。連絡先交換・違法行為・改正風営法 NG ワード等は規約で禁止されています。投稿の責任はご自身にあります。
        </p>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <button type="button" onClick={onCancel} style={{ ...primaryBtn(false), background: 'var(--noxa-surface-hover)', color: 'var(--noxa-text-muted)', border: '1px solid var(--noxa-border)' }}>修正する</button>
