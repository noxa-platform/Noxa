...
export async function POST(request: NextRequest) {
  try {
    ...
    // 🔴 **別リポの消費者**（2026-08-30 に yorulog が実測して接続）:
    // `YoruLog/Team/TeamStatsService.swift:45` の **1 箇所だけ**。3 面（ホーム / ランキング /
    // キャスト一覧）に断り書きを出す。⇒ **`incomplete` に積む文字列を増やすときは、
    // 増やす前に yorulog へ連絡する**（積んだ文字列がそのまま画面に出る。件数と文字数の
    // 上限は向こうで掛けている）。向こうがこの消費を動かすときは連絡が来る。
    // ⚠️ **この日付が古いことは「変わった」ではなく「測り直していない」を意味する**（P162-PM3）。
    // 📌 `"来店ログ"` が落ちた月は**全キャストの売上・組数が丸ごと 0** になる。
    //    読まれていなかった間、その 0 が**給与査定の材料として断りなしに出ていた**（向こうの実測）。
